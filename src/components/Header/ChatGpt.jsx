...
export function ChatGpt() {
    ...
    async function getAllTextFromActiveTab() {
        Logger("Copying Page Content")
        try {
    ...
    async function openChatGPTAndWait() {
        ...
        return new Promise((resolve) => {
            ...
            const listener = (tabId, info) => {
                ...
                console.log("ChatGPT fully loaded ✔");
    ...
    return (
        <>
            <div className="gpt button" onClick={getAllTextFromActiveTab}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="openai-icon icon">
                    <path fill="var(--accent)"
                        d={imagepath}>
                    </path>
                </svg>
                <p className={`logplacement ${logging ? "logging" : ""}`}>
                    {text}
                </p>
            </div>
        </>
